...
parser.add_argument("--img_h",  nargs='?', type=int, help="image height")
parser.add_argument("--img_w",  nargs='?', type=int, help="image width")
parser.add_argument("--frame_num", nargs='?', type=int, help="image number")
args = parser.parse_args()
#EDITED - added optional args and some if statements

start_id = 0
end_id = len([x for x in os.listdir(args.path) if re.match('.+\.jpg', x)])
...
sel_ids = np.arange(0, num_frames, min(40, num_frames-1))
...
sel_ids = np.arange(0, num_frames, max(1, int(num_frames / batch_size)))[:batch_size]
imgs = []
