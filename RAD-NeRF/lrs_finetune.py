...
from nerf.provider import NeRFDataset, NeRFDatasetLRS
...
if __name__ == '__main__':
    ...
    if opt.head_ckpt != '':   
        ...
        # freeze these keys
        print('not freezing loaded model!')
    ...
    train_loader = NeRFDatasetLRS(opt, device=device, type='train').dataloader()
    ...
    # temp fix: for update_extra_states
    if hasattr(train_loader._data, 'speaker_vid_tups'):
        model.speaker_vid_tups = train_loader._data.speaker_vid_tups
    ...
    valid_loader = NeRFDatasetLRS(opt, device=device, type='val', downscale=1).dataloader()
